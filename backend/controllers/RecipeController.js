...
class RecipeController {
  ...
  // POST /api/recipes/search - Search recipes by ingredients using AI service
  static async searchRecipes(req, res) {
    try {
      const { ingredients, limit = 10, query } = req.body;
      
      // Check if AI service is available
      const aiAvailable = await aiService.isAvailable();
      
      if (aiAvailable && (query || ingredients)) {
        try {
          // Use AI service for semantic search
          const searchQuery = query || ingredients.join(' ');
          const results = await aiService.searchRecipes(searchQuery, parseInt(limit), ingredients || []);
          ...
          console.warn('AI service search failed, falling back to local search:', aiError.message);
        }
      }
